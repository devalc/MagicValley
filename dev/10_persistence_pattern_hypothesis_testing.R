...
data <- raster::stack("D:/OneDrive - University of Idaho/MagicValleyData/Relative_ET/Relative_ET_stack_1986_2020.tif")
...
tstattest95 <- tstattest95 %>% dplyr::mutate(difference = mean -1)%>% 
  dplyr::mutate(persistance = case_when(pvalue < 0.05 ~ as.integer(1),
                                        TRUE ~ as.integer(0)))  %>% dplyr::select(persistance, difference, mean)
...
datadf_with_stats <- datadf_with_stats %>% dplyr::mutate(difference_class = dplyr::case_when((persistance_95==1 & difference_95 < 0) ~ as.integer(-1),
